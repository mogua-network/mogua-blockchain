...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("moguavdf").location)' 2>/dev/null)/vdf_client
MOGUAVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("moguavdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing moguavdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary moguavdf "$MOGUAVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary moguavdf "$MOGUAVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing moguavdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing moguavdf requirement boost for MacOS."
		brew install boost
		echo "Installing moguavdf from source."
