...
from mogua.cmds.init_funcs import check_keys, mogua_init
from mogua.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from mogua.daemon.keychain_server import KeychainServer, keychain_commands
...
from mogua.util.keychain import (
    Keychain,
...
from mogua.util.service_groups import validate_service
from mogua.util.setproctitle import setproctitle
from mogua.util.ws_message import WsRpcMessage, create_payload, format_response
...
service_plotter = "mogua plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("mogua daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered MOGUA_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with MOGUA_ROOT: {os.environ['MOGUA_ROOT']}")
    ...
    if service_command == "mogua_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in mogua_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    mogua_init(root_path, should_check_keys=(not wait_for_unlock))
