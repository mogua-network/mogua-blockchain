...
from mogua.rpc.farmer_rpc_client import FarmerRpcClient
...
from mogua.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total mogua farmed: {amounts['farmed_amount'] / units['mogua']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['mogua']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['mogua']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'mogua start wallet' and 'mogua wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'mogua wallet show'")
    else:
        print("Note: log into your key using 'mogua wallet show' to see rewards for each key")
