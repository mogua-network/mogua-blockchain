...
from mogua.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_mogua_ca_crt_key,
    make_ca_cert,
...
from mogua.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'mogua keys generate'")
        return None
...
def create_all_ssl(root_path: Path):
    ...
    write_ssl_cert_and_key(mogua_ca_crt_path, mogua_ca_crt, mogua_ca_key_path, mogua_ca_key)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if mogua_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return mogua_init(root_path, fix_ssl_permissions=fix_ssl_permissions)
...
def mogua_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
    ...
    """
    if os.environ.get("MOGUA_ROOT", None) is not None:
        print(
            f"warning, your MOGUA_ROOT is set to {os.environ['MOGUA_ROOT']}. "
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'mogua keys show --show-mnemonic-seed'")
