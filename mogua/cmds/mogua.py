...
from mogua.cmds.configure import configure_cmd
from mogua.cmds.farm import farm_cmd
from mogua.cmds.init import init_cmd
from mogua.cmds.keys import keys_cmd
from mogua.cmds.netspace import netspace_cmd
from mogua.cmds.passphrase import passphrase_cmd
from mogua.cmds.plots import plots_cmd
from mogua.cmds.show import show_cmd
from mogua.cmds.start import start_cmd
from mogua.cmds.stop import stop_cmd
from mogua.cmds.wallet import wallet_cmd
from mogua.cmds.plotnft import plotnft_cmd
from mogua.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from mogua.util.keychain import set_keys_root_path, supports_keyring_passphrase
from mogua.util.ssl import check_ssl
...
@click.group(
    help=f"\n  Manage mogua blockchain infrastructure ({__version__})\n",
    epilog="Try 'mogua start node', 'mogua netspace -d 192', or 'mogua show -s'",
...
@cli.command("version", short_help="Show mogua version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs mogua daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by mogua start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from mogua.daemon.server import async_run_daemon
