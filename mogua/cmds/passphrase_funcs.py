...
from mogua.util.keychain import Keychain, obtain_current_passphrase
from mogua.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'mogua passphrase set' or 'mogua passphrase remove' to update or remove your passphrase")
        sys.exit(1)
...
async def async_update_daemon_passphrase_cache_if_running(root_path: Path) -> None:
    from mogua.daemon.client import connect_to_daemon_and_validate
