...
from mogua.server.start_wallet import SERVICE_NAME
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_amount = uint64(int(amount * units["mogua"]))
    final_fee = uint64(int(fee * units["mogua"]))
    ...
    while time.time() - start < 10:
        ...
        if len(tx.sent_to) > 0:
            ...
            print(f"Do mogua wallet get_transaction -f {fingerprint} -tx 0x{tx_id} to get status")
            return None
...
async def print_balances(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    for summary in summaries_response:
        ...
        if typ != "STANDARD_WALLET":
            ...
            print(
                f"   -Total Balance: {balances['confirmed_wallet_balance']/units['mogua']} {address_prefix} "
                f"({balances['confirmed_wallet_balance']} mojo)"
            )
            print(
                f"   -Pending Total Balance: {balances['unconfirmed_wallet_balance']/units['mogua']} {address_prefix} "
                f"({balances['unconfirmed_wallet_balance']} mojo)"
            )
            print(
                f"   -Spendable: {balances['spendable_balance']/units['mogua']} {address_prefix} "
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'mogua keys generate' or import a key")
        return None
