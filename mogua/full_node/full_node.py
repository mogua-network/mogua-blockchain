...
from mogua.consensus.blockchain_interface import BlockchainInterface
...
from mogua.util.check_fork_next_block import check_fork_next_block
...
class FullNode:
    ...
    def set_server(self, server: MoguaServer):
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.mogua.net")
        try:
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSMoguaConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSMoguaConnection, List[FullBlock]]] = asyncio.Queue(
...
async def node_next_block_check(
    peer: ws.WSMoguaConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
