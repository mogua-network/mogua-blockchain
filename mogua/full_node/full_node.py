...
import mogua.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from mogua.consensus.block_creation import unfinished_block_to_full_block
from mogua.consensus.block_record import BlockRecord
from mogua.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from mogua.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from mogua.consensus.multiprocess_validation import PreValidationResult
from mogua.consensus.pot_iterations import calculate_sp_iters
...
from mogua.full_node.full_node_store import FullNodeStore
from mogua.full_node.mempool_manager import MempoolManager
from mogua.full_node.signage_point import SignagePoint
from mogua.full_node.sync_store import SyncStore
from mogua.full_node.weight_proof import WeightProofHandler
from mogua.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from mogua.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from mogua.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from mogua.util.path import mkdir, path_from_root
from mogua.util.safe_cancel_task import cancel_task_safe
from mogua.util.profiler import profile_task
...
class FullNode:
    ...
    def set_server(self, server: MguaServer):
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.mogua.gua")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSMguaConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSMguaConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSMguaConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSMguaConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSMguaConnection):
        """
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSMguaConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSMguaConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSMguaConnection],
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'mga')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSMguaConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSMguaConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSMguaConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSMguaConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSMguaConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
