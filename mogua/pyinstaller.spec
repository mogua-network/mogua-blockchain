...
ROOT = pathlib.Path(importlib.import_module("mogua").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `mogua` file name (which is the executable)
    and the `mogua` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `mogua` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("mogua/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("mogua-blockchain"))[0]
...
datas.append((f"{ROOT}/mogua/util/english.txt", "mogua/util"))
datas.append((f"{ROOT}/mogua/util/initial-config.yaml", "mogua/util"))
datas.append((f"{ROOT}/mogua/wallet/puzzles/*.hex", "mogua/wallet/puzzles"))
datas.append((f"{ROOT}/mogua/ssl/*", "mogua/ssl"))
...
add_binary("mogua", f"{ROOT}/mogua/cmds/mogua.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/mogua/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/mogua/server/start_{server}.py", COLLECT_ARGS)
