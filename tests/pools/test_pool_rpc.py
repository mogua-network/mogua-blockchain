...
from mogua.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from mogua.protocols import full_node_protocol
from mogua.protocols.full_node_protocol import RespondBlock
from mogua.rpc.rpc_server import start_rpc_server
from mogua.rpc.wallet_rpc_api import WalletRpcApi
from mogua.rpc.wallet_rpc_client import WalletRpcClient
from mogua.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestPoolWalletRpc:
    ...
    async def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32, shuil=None) -> bytes32:
        ...
        try:
            plot_keys = PlotKeys(bt.farmer_pk, None, encode_puzzle_hash(p2_singleton_puzzle_hash, "tmga"))
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc):
        ...
        tr: TransactionRecord = await client.send_transaction(
            1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "tmga")
        )
        await time_out_assert(
            10,
            full_node_api.full_node.mempool_manager.get_spendbundle,
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc):
        ...
        # Balance stars at 6 MGA and 5 more blocks are farmed, total 22 MGA
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_mogua():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_mogua)
