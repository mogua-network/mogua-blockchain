...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	#PLATFORM="$1"
	DIR_NAME="mogua-blockchain-linux-arm64"
fi
...
# sets the version for mogua-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$MOGUA_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	...
	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Mogua-Network/mogua-blockchain/issues/3846
