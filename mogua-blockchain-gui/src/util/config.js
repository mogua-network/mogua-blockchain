...
global.daemon_rpc_ws = `wss://${self_hostname}:55795`;
...
function loadConfig(net) {
  try {
    // check if MOGUA_ROOT is set. it overrides 'net'
    const config_root_dir =
      'MOGUA_ROOT' in process.env
        ? process.env.MOGUA_ROOT
...
//    const daemon_port = lodash.get(config, 'ui.daemon_port', 55795); // hardcoded
    const daemon_port = '55795'; // hardcoded
