...
- Added a py.typed file that allows other projects that pip install mogua-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the mogua install and then converts it into proper npm format and puts that into package.json.
- Added some new class methods to the Program objects to improve ease of use.
- Added an option to sign bytes as well as UTF-8 strings, which is particularly helpful if you're writing Mogualisp puzzles that require signatures and you want to test them without necessarily writing a whole python script for signing the relevant data.
...
## 1.2.3 Mogua blockchain 2021-07-26
...
- Thanks @mishan for your change to 'mogua plotnft show' to display Percent Successful Points.
...
## 1.2.0 Mogua blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `mogua plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Mogua-Network/mogua-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Mogua-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `mogua farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `mogua configure` now supports command line updates to peer count and target peer count.
...
- New CLI command `mogua plotnft` to manage pools.
...
- Correctly display private key in `mogua keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `mogua keys show`.
...
- Thank you to @MoguaMineJP for various improvements.
...
- `mogua farm summary` better handles wallet errors.
...
- You should review the [release notes for v1.0.2](https://github.com/Mogua-Network/mogua-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- This is a hard fork/breaking change from RC6/7. TMGA Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.mogua.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Mogua-Network/mogua-blockchain/issues/1189)
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Mogua-Network/mogua-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `mogua -h` and `mogua SUBCOMMAND -h` can be your guide.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Mogua-Network/mogua-blockchain/releases/tag/1.0rc1) was well.
...
- There is now remote UI support. [Documents](https://github.com/Mogua-Network/mogua-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Mogua-Network/mogua-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Mogua-Network/mogua-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Mogua-Network/mogua-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Mogua-Network/mogua-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- Testnet is now running at the primary port of 56935. Update your routers appropriately. This opens 6935 for mainnet.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Mogua-Network/mogua-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/Mogua-Network/mogua-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Mogua-Network/mogua-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Mogua-Network/mogua-blockchain/tree/dev/electron-react/src/locales) of the mogua-blockchain repository.
...
- Fixed a typo in `mogua show -w` with thanks to @pyl on Keybase.
...
are read.
- Updated [blspy](https://github.com/Mogua-Network/bls-signatures) to use the
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Mogua-Network/mogua-blockchain/wiki) and placed in [INSTALL](https://github.com/Mogua-Network/mogua-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Mogua-Network/mogua-blockchain/wiki/Quick-Start-Guide)
...
- We suggest you take a look at our [Upgrading documentation](https://github.com/Mogua-Network/mogua-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Mogua-Network/mogua-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- There is a new command `mogua init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `mogua-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Mogua-Network/mogua-blockchain/wiki/Updating-beta-software). If you've set `$MOGUA_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- There is now full transaction support on the Mogua blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Mogua-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
[1.0beta4]: https://github.com/Mogua-Network/mogua-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Mogua-Network/mogua-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Mogua-Network/mogua-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Mogua-Network/mogua-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Mogua-Network/mogua-blockchain/releases/tag/Alpha-1.0
