...
## 1.2.2 MoGua blockchain 2021-07-13

### Fixed

- Thanks to @yshklarov for help fixing [#7273](https://github.com/MoGua-Network/mogua-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 MoGua blockchain 2021-07-12
...
- All MoGua repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
...
- We have increased the strictness of validating MoGualisp in the mempool and clvm.
...
### 1.1.7 MoGua Blockchain 2021-06-05
...
## 1.1.6 MoGua Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /mogua/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `mogua configure -t true|false`).
...
## 1.1.5 MoGua Blockchain 2021-05-09
...
## 1.1.4 MoGua Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `mogua keys show`. Use `mogua keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 MoGua Blockchain 2021-05-01
...
- We changed mogualisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
## 1.1.2 MoGua Blockchain 2021-04-24
...
## 1.1.1 MoGua Blockchain 2021-04-21
...
## 1.1.0 MoGua Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the MoGua Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of MoGualisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [mogualisp.com](https://mogualisp.com) shortly.
...
## 1.0.5 MoGua Blockchain 2021-04-14
...
## 1.0.4 MoGua Blockchain 2021-04-12
...
- The `src` directory in mogua-blockchain has been changed to `mogua` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `mogua/`
...
## 1.0.3 MoGua Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/MoGua/mogua-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- We have released version 1.0.0 of [chiapos](https://github.com/MoGua/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- @jespino changed `mogua keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `mogua keys show`.
...
## 1.0.1 MoGua Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `mogua keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/mogua-blockchain/).
...
- Users can now pip install e.g. mogua-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of MoGua Blockchain 2021-03-17
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/mogua-blockchain).
...
- Migration with MOGUA_ROOT set does not crash mogua init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TMGA Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.mogua.xyz/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `mogua farm summary` displayed some incorrect amounts.
...
- `mogua netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `mogua netspace`. Thanks @altendky.
- There was a typo in `mogua plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/MoGua/mogua-blockchain/issues/1189)
...
- We now hand build Apple Silicon native binary wheels for all mogua-blockchain dependencies and host them at [https://pypi.mogua.xyz/simple](https://pypi.mogua.xyz/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a mogualisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during MoGuaLisp native deserialization.
...
- We have moved to a new and much higher scalability download.mogua.xyz to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.mogua.xyz/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.mogua.xyz/latest/Setup-MacOS.dmg).
- MoGua wheels not on Pypi and some dependecies not found there also are now on pypi.mogua.xyz.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.mogua.xyz/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- `mogua wallet` now takes get_address to get a new wallet receive address from the CLI.
- `mogua plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- MoGualisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of mogua-blockchain, etc., instead of in blspy.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.mogua.xyz via https, for example.
...
- Various typos and style clean ups were made to the Click CLI implementation. `mogua -upnp f` was added to disable uPnP.
- `mogua plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/mogua-blockchain).
- You can obtain a new wallet receive address on the command line with `mogua wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All mogualisp opcodes have been renumbered. This should be the last major breaking change for mogualisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/MoGua/mogua-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `mogua -h` and `mogua SUBCOMMAND -h` can be your guide.
...
- `mogua plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `mogua netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `mogua init` only migrates release candidate directories. The versioned sub directories under `~/mogua` will be going away before mainnet.
...
- `mogua configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/MoGua/mogua-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and mogualisp are in release form. We have one major change to mogualisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern MoGuaSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'mogua wallet get_transactions' in the command line to see your transactions.
- 'mogua wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [MoGua Blockchain GUI](https://crowdin.com/project/mogua-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/MoGua/mogua-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/MoGua/mogua-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `mogua plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'mogua wallet send' now takes in TMGA or MGA as units instead of mojos.
...
- The GUI is now in a separate submodule repository from mogua-blockchain, [mogua-blockchain-gui](https://github.com/MoGua/mogua-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `mogua-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- `mogua plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.

### Changed

- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 TMGA as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/MoGua/clvm_rs) is complete.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/MoGua/mogua-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated MoGualisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/MoGua/mogua-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/MoGua/mogua-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `mogua plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and MoGuaLisp code matches the CI compiled code.
- We have removed the '-r' flag from `mogua` as it was being overridden in most cases by the `-r` for restart flag to `mogua start`. Use `mogua --root-path` instead.
- `mogua -h` now recommends `mogua netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `mogua show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `mogua configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `mogua plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `mogua plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `mogua show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `mogua show` handles sub block 0 better.
...
- `mogua plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `mogua plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `mogua show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `mogua configure --enable-upnp`.
...
- `mogua configure` now lets you enable or disable uPnP.
...
- `mogua netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `mogua show` of the GUI use `mogua netspace -d 1000` as `mogua netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [mogua.xyz](https://mogua.xyz/). Among the improvements this gives the MoGua blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- `mogua show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/MoGua/mogua-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install mogua-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/MoGua/mogua-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/MoGua/mogua-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The MoGua GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/MoGua/mogua-blockchain/tree/dev/electron-react/src/locales) of the mogua-blockchain repository.
- `mogua check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/MoGua/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all mogua services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test mogua since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve mogua-blockmoguan's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from MoGua Network at backup.mogua.xyz. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the MoGua backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.mogua.xyz, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test mogua are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.mogua` directory.
...
- `mogua init` now automatically discovers previous releases in each new release.

### Fixed

- `mogua show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `mogua show -w` with thanks to @pyl on Discord.
...
- The MoGua UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (mogua run farmer-only, wallet-only, timelord-only)
...
- `mogua show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `mogua show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the MoGua simple site. Raspberry Pi should be just a little easier to install.
...
- See wallet balances in command line: `mogua show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `mogua keys sign` and `mogua keys verify` to allow farmers to certify their ownership of keys.
...
- `mogua start wallet-server` changed to `mogua start wallet`, for consistency.
...
- Removed legacy scripts such as mogua-stop-server, mogua-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/MoGua/bls-signatures) to use the
...
- The mogua executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./mogua -h` from
`~\AppData\Local\MoGua-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/MoGua.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"mogua plots create" and "mogua plots check".
- Add plot directories to config.yaml from the cli with "mogua plots add".
...
- Test plots for the simulation and testing harness now go into `~/.mogua/test-plots/`
- We have completely refactored all networking code towards making each MoGua service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make mogua start/stop reliable cross platform.
...
- Starting mogua now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'mogua keys' command line.
...
- We added a `mogua keys` command line program, to see, add, and remove private keys.
...
- `mogua netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `mogua-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `mogua-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- There was a regression in MoGua Proof of Space ([chiapos](https://github.com/MoGua/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `mogua init` then sync on first start will happen automatically. "\$MOGUA_ROOT" users will need to delete `$MOGUA_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `mogua netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during mogua init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `mogua show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `mogua show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/MoGua/mogua-blockchain/wiki) and placed in [INSTALL](https://github.com/MoGua/mogua-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/MoGua/mogua-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of mogua action commands. You can run `mogua version` or `mogua start node` for example. Just running `mogua` will show you more options. However `mogua-create-plots` continues to use the hyphenated form. Also it's now `mogua generate keys` as another example.
- MoGua start commands like `mogua start farmer` and `mogua stop node` now keep track of process IDs in a run/ directory in your configuration directory. `mogua stop` is unlikely to work on Windows native for now. If `mogua start -r node` doesn't work you can force the run/ directory to be reset with `mogua start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/MoGua/mogua-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `mogua start wallet-gui` has been chaned to `mogua start wallet` which launches but the GUI and server on MacOS and Linux. `mogua start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/MoGua/mogua-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `mogua stop all` command sometimes fails, use `mogua-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `mogua init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `mogua-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/MoGua/mogua-blockchain/wiki/Updating-beta-software). If you've set `$MOGUA_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `mogua show`.

### Changed

- `mogua-restart-harvester` has been renamed from `mogua-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.mogua/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the MoGua blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/MoGua/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the mogua management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `mogua-start-farmer &`. The [README.md](https://github.com/MoGua/mogua-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- The libraries mogua-pos, mogua-fast-vdf, and mogua-bip-158 have been moved to their own repositories: [chiapos](https://github.com/MoGua/chiapos), [chiavdf](https://github.com/MoGua/chiavdf), and [chaibip158](https://github.com/MoGua/chiabip158). They are brought in by mogua-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/MoGua/bls-signatures).
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:6948/index.html](http://127.0.0.1:6948/index.html) with any modern web browser on the same machine as the full node.
...
- MoGua fullnode, farmer and harvester now default to logging to mogua.log in the mogua-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Implemented an RPC interface with JSON serialization for streamables - currently on port 6948.
...
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to mogua_full node, mogua_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new mogua services were started.
...
- More details on the release at [https://www.mogua.xyz/developer/](https://www.mogua.xyz/developer/)

[unreleased]: https://github.com/MoGua/mogua-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/MoGua/mogua-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/MoGua/mogua-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/MoGua/mogua-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/MoGua/mogua-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/MoGua/mogua-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/MoGua/mogua-blockchain/releases/tag/Alpha-1.0
